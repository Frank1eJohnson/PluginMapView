...
bool FOSMFile::LoadOpenPluginMapViewFile( FString& OSMFilePath, const bool bIsFilePathActuallyTextBuffer, FFeedbackContext* FeedbackContext )
{
	...
	else if( ParsingState == ParsingState::Way_Tag )
	{
		...
		else if( !FCString::Stricmp( AttributeName, TEXT( "v" ) ) )
		{
			...
			else if( !FCString::Stricmp( CurrentWayTagKey, TEXT( "highway" ) ) )
			{
				...
				{
					// Other type that we don't recognize yet.  See http://wiki.openpluginmapview.org/wiki/Key:highway
				}
			...
			else if( !FCString::Stricmp( CurrentWayTagKey, TEXT( "building" ) ) )
			{
				...
				{
					// Other type that we don't recognize yet.  See http://wiki.openpluginmapview.org/wiki/Key:building
				}
			}
