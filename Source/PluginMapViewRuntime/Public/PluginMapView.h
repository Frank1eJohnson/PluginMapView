...
#include "PluginMapView.generated.h"


USTRUCT(BlueprintType)
struct PLUGINMAPVIEWRUNTIME_API FPluginMapViewCollisionSettings
{
...
public:
	...
	*/
	UPROPERTY(EditAnywhere, Category = "PluginMapView", meta = (editcondition = "bGenerateCollision"))
		uint32 bAllowDoubleSidedGeometry : 1;


	FPluginMapViewCollisionSettings() :
...
USTRUCT(BlueprintType)
struct PLUGINMAPVIEWRUNTIME_API FPluginMapViewMeshBuildSettings
{
	GENERATED_USTRUCT_BODY()
...
public:
	...
	UPROPERTY(Category = PluginMapView, EditAnywhere, meta = (ClampMin = "0", UIMin = "0"), DisplayName = "Road Vertical Offset")
		float RoadOffesetZ;

	/** if true buildings mesh will be 3D instead of flat representation. */
	UPROPERTY(Category = PluginMapView, EditAnywhere, DisplayName = "Create 3D Buildings")
	...
	*/
	UPROPERTY(Category = PluginMapView, EditAnywhere, DisplayName = "Building Level Floor Factor")
	...
	*/
	UPROPERTY(Category = PluginMapView, EditAnywhere, DisplayName = "Lit buildings")
	uint32 bWantLitBuildings : 1;

	/** Streets thickness */
	UPROPERTY(Category = PluginMapView, EditAnywhere, meta = (ClampMin = "0", UIMin = "0"))
		float StreetThickness;

	/** Street vertex color */
	UPROPERTY(Category = PluginMapView, EditAnywhere)
	...
	FPluginMapViewMeshBuildSettings() :
		RoadOffesetZ(0.0f),
...
/** A road */
USTRUCT( BlueprintType )
struct PLUGINMAPVIEWRUNTIME_API FPluginMapViewRoad
{
	GENERATED_USTRUCT_BODY()
	...
	/** Type of road */
	UPROPERTY( Category=PluginMapView, EditAnywhere )
	TEnumAsByte<EPluginMapViewRoadType> RoadType;
	...
	inline int32 GetRoadIndex( const class UPluginMapView& PluginMapView ) const;

	/** Gets the node for the specified point, or the node that came before that if the specified point doesn't have a node */
	inline const struct FPluginMapViewNode& GetNodeAtPointIndexOrEarlier( const class UPluginMapView& PluginMapView, const int32 PointIndex, int32& OutNodeAtPointIndex ) const;

	/** Gets the node for the specified point, or the node that comes next after that if the specified point doesn't have a node */
	inline const struct FPluginMapViewNode& GetNodeAtPointIndexOrLater( const class UPluginMapView& PluginMapView, const int32 PointIndex, int32& OutNodeAtPointIndex ) const;

	/** Computes the total length of this road by following along all of it's points */
	float ComputeLengthOfRoad( const class UPluginMapView& PluginMapView ) const;

	/** Computes the distance along the road between two points on the road.  Be careful!  The same node can appear on a road twice. */
	float ComputeDistanceBetweenNodesOnRoad( const class UPluginMapView& PluginMapView, const int32 NodePointIndexA, const int32 NodePointIndexB ) const;

	/** Given a position along the road, finds the nodes that come earlier and later on that road */
	void FindEarlierAndLaterNodesForPositionAlongRoad( const class UPluginMapView& PluginMapView, const float PositionAlongRoad, const FPluginMapViewNode*& OutEarlierNode, float& OutEarlierNodePositionAlongRoad, const FPluginMapViewNode*& OutLaterNode, float& OutLaterNodePositionAlongRoad ) const;
	...
	void FindEarlierAndLaterNodes( const class UPluginMapView& PluginMapView, const int32 RoadPointIndex, const FPluginMapViewNode*& OutEarlierNode, float& OutEarlierNodePositionAlongRoad, const FPluginMapViewNode*& OutLaterNode, float& OutLaterNodePositionAlongRoad ) const;
	...
	float FindPositionAlongRoadForNode( const class UPluginMapView& PluginMapView, const int32 PointIndexForNode ) const;

	/** Computes the location of a point along this road, given a distance along this road from the road's beginning */
	FVector2D MakeLocationAlongRoad( const class UPluginMapView& PluginMapView, const float PositionAlongRoad ) const;
...
USTRUCT( BlueprintType )
struct PLUGINMAPVIEWRUNTIME_API FPluginMapViewRoadRef
{
	GENERATED_USTRUCT_BODY()
...
USTRUCT( BlueprintType )
struct PLUGINMAPVIEWRUNTIME_API FPluginMapViewNode
{
	GENERATED_USTRUCT_BODY()
	...
	UPROPERTY( Category=PluginMapView, EditAnywhere )
	TArray<FPluginMapViewRoadRef> RoadRefs;

	/** Returns this node's index */
	inline int32 GetNodeIndex( const UPluginMapView& PluginMapView ) const;

	/** Gets the location of this node */
	inline FVector2D GetLocation( const UPluginMapView& PluginMapView ) const;
	...
	inline const FPluginMapViewRoad& GetShortestCostRoadToNode( UPluginMapView& PluginMapView, const FPluginMapViewNode& OtherNode, const bool bIsTravelingForward, int32& OutPointIndexOnRoad ) const;

	/** Pathfinding: Returns true if this node is the end point on a road with no connections */
	inline bool IsDeadEnd( const UPluginMapView& PluginMapView ) const;
	...
	inline int32 GetConnectionCount( const class UPluginMapView& PluginMapView, const bool bIsTravelingForward ) const;
	...
	inline const FPluginMapViewNode* GetConnection( const class UPluginMapView& PluginMapView, const int32 ConnectionIndex, const bool bIsTravelingForward, const struct FPluginMapViewRoad** OutConnectingRoad = nullptr, int32* OutPointIndexOnRoad = nullptr, int32* OutConnectedNodePointIndexOnRoad = nullptr ) const;

	/** Pathfinding: Estimates the 'cost' of the specified connected by index (between 0 and GetConnectionCount() - 1) */
	inline float GetConnectionCost( const class UPluginMapView& PluginMapView, const int32 ConnectionIndex, const bool bIsTravelingForward ) const;
};


/** A building */
USTRUCT( BlueprintType )
struct PLUGINMAPVIEWRUNTIME_API FPluginMapViewBuilding
{
	GENERATED_USTRUCT_BODY()
...
/** A loaded street map */
UCLASS()
class PLUGINMAPVIEWRUNTIME_API UPluginMapView : public UObject
{
	GENERATED_BODY()
	
public:

	/** Default constructor for UPluginMapView */
	UPluginMapView();
	...
	const TArray<FPluginMapViewRoad>& GetRoads() const
	{
		return Roads;
	}

	/** Gets the roads in this street map */
	TArray<FPluginMapViewRoad>& GetRoads()
	{
		return Roads;
	}
	
	/** Gets the nodes on the map (read only.)  Nodes describe intersections between roads */
	const TArray<FPluginMapViewNode>& GetNodes() const
	{
		return Nodes;
	}

	/** Gets the nodes on the map.  Nodes describe intersections between roads */
	TArray<FPluginMapViewNode>& GetNodes()
	{
		return Nodes;
	}
	
	/** Gets all of the buildings (read only) */
	const TArray<FPluginMapViewBuilding>& GetBuildings() const
	{
		return Buildings;
	}

	/** Gets all of the buildings */
	TArray<FPluginMapViewBuilding>& GetBuildings()
	{
		return Buildings;
	}
...
protected:
	...
	/** List of roads */
	UPROPERTY( Category=PluginMapView, VisibleAnywhere )
	TArray<FPluginMapViewRoad> Roads;
	...
	UPROPERTY( Category=PluginMapView, VisibleAnywhere )
	TArray<FPluginMapViewNode> Nodes;

	/** List of all buildings on the street map */
	UPROPERTY( Category=PluginMapView, VisibleAnywhere)
	TArray<FPluginMapViewBuilding> Buildings;
	...
	friend class UPluginMapViewFactory;
	friend class UPluginMapViewReimportFactory;
	friend class FPluginMapViewAssetTypeActions;
#endif	// WITH_EDITORONLY_DATA

};


inline int32 FPluginMapViewRoad::GetRoadIndex( const UPluginMapView& PluginMapView ) const
{
	// Pointer arithmetic based on array start
	const int32 RoadIndex = this - PluginMapView.GetRoads().GetData();
	return RoadIndex;
}


inline const FPluginMapViewNode& FPluginMapViewRoad::GetNodeAtPointIndexOrEarlier( const UPluginMapView& PluginMapView, const int32 PointIndex, int32& OutNodeAtPointIndex ) const
{
	const FPluginMapViewNode* CurrentOrEarlierPointNode = nullptr;
	for( int32 NodePointIndex = PointIndex; NodePointIndex >= 0; --NodePointIndex )
	{
		if( NodeIndices[ NodePointIndex ] != INDEX_NONE )
		{
			CurrentOrEarlierPointNode = &PluginMapView.GetNodes()[ NodeIndices[ NodePointIndex ] ];
...
inline const FPluginMapViewNode& FPluginMapViewRoad::GetNodeAtPointIndexOrLater( const UPluginMapView& PluginMapView, const int32 PointIndex, int32& OutNodeAtPointIndex ) const
{
	const FPluginMapViewNode* NextOrUpcomingNode = nullptr;
	for( int32 NodePointIndex = PointIndex; NodePointIndex < RoadPoints.Num(); ++NodePointIndex )
	{
		if( NodeIndices[ NodePointIndex ] != INDEX_NONE )
		{
			NextOrUpcomingNode = &PluginMapView.GetNodes()[ NodeIndices[ NodePointIndex ] ];
			OutNodeAtPointIndex = NodePointIndex;
			break;
		}
	}
...
inline float FPluginMapViewRoad::ComputeLengthOfRoad( const class UPluginMapView& PluginMapView ) const
{
	...
	return ComputeDistanceBetweenNodesOnRoad( PluginMapView, 0, this->NodeIndices.Num() - 1 );
}


inline float FPluginMapViewRoad::ComputeDistanceBetweenNodesOnRoad( const class UPluginMapView& PluginMapView, const int32 NodePointIndexA, const int32 NodePointIndexB ) const
{
...
inline void FPluginMapViewRoad::FindEarlierAndLaterNodesForPositionAlongRoad( const class UPluginMapView& PluginMapView, const float PositionAlongRoad, const FPluginMapViewNode*& OutEarlierNode, float& OutEarlierNodePositionAlongRoad, const FPluginMapViewNode*& OutLaterNode, float& OutLaterNodePositionAlongRoad ) const
{
	float CurrentPointPositionAlongRoad = 0.0f;

	const FPluginMapViewNode* EarlierPluginMapViewNode = nullptr;
	const FPluginMapViewNode* LaterPluginMapViewNode = nullptr;
	...
	for( int32 CurrentPointIndex = 0; CurrentPointIndex < NumPoints - 1; ++CurrentPointIndex )
	{
		...
		{
			EarlierPluginMapViewNode = &PluginMapView.GetNodes()[ this->NodeIndices[ CurrentPointIndex ] ];
		...
		if( NextPointPositionAlongRoad >= PositionAlongRoad )
		{
			...
			{
				LaterPluginMapViewNode = &PluginMapView.GetNodes()[ this->NodeIndices[ NextPointIndex ] ];
	...
	check( EarlierPluginMapViewNode != nullptr && LaterPluginMapViewNode != nullptr );
	OutEarlierNode = EarlierPluginMapViewNode;
	OutLaterNode = LaterPluginMapViewNode;
}


inline void FPluginMapViewRoad::FindEarlierAndLaterNodes( const class UPluginMapView& PluginMapView, const int32 RoadPointIndex, const FPluginMapViewNode*& OutEarlierNode, float& OutEarlierNodePositionAlongRoad, const FPluginMapViewNode*& OutLaterNode, float& OutLaterNodePositionAlongRoad ) const
{
	...
	for( int32 EarlierPointIndex = RoadPointIndex - 1; EarlierPointIndex >= 0; --EarlierPointIndex )
	{
		...
		{
			OutEarlierNode = &PluginMapView.GetNodes()[ this->NodeIndices[ EarlierPointIndex ] ];
			OutEarlierNodePositionAlongRoad = FindPositionAlongRoadForNode( PluginMapView, EarlierPointIndex );
			break;
		}
	}
	...
	for( int32 LaterPointIndex = RoadPointIndex + 1; LaterPointIndex < this->RoadPoints.Num(); ++LaterPointIndex )
	{
		...
		{
			OutLaterNode = &PluginMapView.GetNodes()[ this->NodeIndices[ LaterPointIndex ] ];
			OutLaterNodePositionAlongRoad = FindPositionAlongRoadForNode( PluginMapView, LaterPointIndex );
			break;
		}
	}
}
...
inline float FPluginMapViewRoad::FindPositionAlongRoadForNode( const class UPluginMapView& PluginMapView, const int32 PointIndexForNode ) const
{
	float CurrentPointPositionAlongRoad = 0.0f;
...
inline FVector2D FPluginMapViewRoad::MakeLocationAlongRoad( const class UPluginMapView& PluginMapView, const float PositionAlongRoad ) const
{
...
inline int32 FPluginMapViewNode::GetNodeIndex( const UPluginMapView& PluginMapView ) const
{
	// Pointer arithmetic based on array start
	const int32 NodeIndex = this - PluginMapView.GetNodes().GetData();
	return NodeIndex;
}


inline bool FPluginMapViewNode::IsDeadEnd( const UPluginMapView& PluginMapView ) const
{
	if( RoadRefs.Num() == 1 )
	{
		...
		const FPluginMapViewRoadRef& SoleRoadRef = RoadRefs[ 0 ];
		const FPluginMapViewRoad& SoleRoad = PluginMapView.GetRoads()[ SoleRoadRef.RoadIndex ];
...
inline FVector2D FPluginMapViewNode::GetLocation( const UPluginMapView& PluginMapView ) const
{
	const FPluginMapViewRoadRef& MyFirstRoadRef = RoadRefs[ 0 ];
	const FVector2D Location = PluginMapView.GetRoads()[ MyFirstRoadRef.RoadIndex ].RoadPoints[ MyFirstRoadRef.RoadPointIndex ];
	return Location;
}


inline const FPluginMapViewRoad& FPluginMapViewNode::GetShortestCostRoadToNode( UPluginMapView& PluginMapView, const FPluginMapViewNode& OtherNode, const bool bIsTravelingForward, int32& OutPointIndexOnRoad ) const
{
	const FPluginMapViewRoad* ConnectingRoad = nullptr;
	...
	const int32 ConnectionCount = GetConnectionCount( PluginMapView, bIsTravelingForward );
	for( int32 ConnectionIndex = 0; ConnectionIndex < ConnectionCount; ++ConnectionIndex )
	{
		...
		const FPluginMapViewRoad* CurrentConnectingRoad;
		const FPluginMapViewNode* ConnectedNode = GetConnection( PluginMapView, ConnectionIndex, bIsTravelingForward, /* Out */ &CurrentConnectingRoad, /* Out */ &MyPointIndexOnRoad, /* Out */ &ConnectedNodePointIndexOnRoad );
		if( ConnectedNode == &OtherNode )
		{
			if( BestConnectionIndex != INDEX_NONE )
			{
				...
				BestConnectionCost = GetConnectionCost( PluginMapView, BestConnectionIndex, bIsTravelingForward );
				if( GetConnectionCost( PluginMapView, ConnectionIndex, bIsTravelingForward ) < BestConnectionCost )
				{
...
inline int32 FPluginMapViewNode::GetConnectionCount( const UPluginMapView& PluginMapView, const bool bIsTravelingForward ) const
{
	// NOTE: We're iterating here in the exact same order as in the GetConnection() function below!  That's critically important!
	int32 TotalConnections = 0;
	for( const FPluginMapViewRoadRef& RoadRef : RoadRefs )
	{
		const FPluginMapViewRoad& Road = PluginMapView.GetRoads()[ RoadRef.RoadIndex ];
...
inline const FPluginMapViewNode* FPluginMapViewNode::GetConnection( const UPluginMapView& PluginMapView, const int32 ConnectionIndex, const bool bIsTravelingForward, const FPluginMapViewRoad** OutConnectingRoad, int32* OutPointIndexOnRoad, int32* OutConnectedNodePointIndexOnRoad ) const
{
	if( OutConnectingRoad != nullptr )
	{
		*OutConnectingRoad = nullptr;
	}
	const FPluginMapViewNode* ConnectedNode = nullptr;
	...
	for( const FPluginMapViewRoadRef& RoadRef : RoadRefs )
	{
		...
		if( RoadRef.RoadPointIndex > 0 && ( !bIsTravelingForward || !Road.IsOneWay() ) )
		{
			...
			if( CurrentConnectionIndex == ConnectionIndex )
			{
				...
				const FPluginMapViewNode& EarlierNode = PluginMapView.GetNodes()[ EarlierNodeIndex ];
		...
		if( RoadRef.RoadPointIndex < ( Road.NodeIndices.Num() - 1 ) && ( bIsTravelingForward || !Road.IsOneWay() ) )
		{
			...
			if( CurrentConnectionIndex == ConnectionIndex )
			{
				...
				const FPluginMapViewNode& LaterNode = PluginMapView.GetNodes()[ LaterNodeIndex ];
...
inline float FPluginMapViewNode::GetConnectionCost( const UPluginMapView& PluginMapView, const int32 ConnectionIndex, const bool bIsTravelingForward ) const
{
	/////////////////////////////////////////////////////////
	...
	const FPluginMapViewRoad* ConnectingRoad = nullptr;
	const FPluginMapViewNode& ConnectedNode = *GetConnection( PluginMapView, ConnectionIndex, bIsTravelingForward, /* Out */ &ConnectingRoad, /* Out */ &MyPointIndexOnRoad, /* Out */ &ConnectedNodePointIndexOnRoad );

	const float DistanceBetweenNodes = ConnectingRoad->ComputeDistanceBetweenNodesOnRoad( PluginMapView, MyPointIndexOnRoad, ConnectedNodePointIndexOnRoad );
	...
	// Apply some scaling to the cost of traveling between these nodes
	{
		...
		{
			case EPluginMapViewRoadType::Highway:
				SpeedLimit = HighwaySpeed;
				TrafficFactor = HighwayTrafficFactor;
				break;

			case EPluginMapViewRoadType::MajorRoad:
				SpeedLimit = MajorRoadSpeed;
				TrafficFactor = MajorRoadTrafficFactor;
				break;

			case EPluginMapViewRoadType::Street:
			case EPluginMapViewRoadType::Other:
