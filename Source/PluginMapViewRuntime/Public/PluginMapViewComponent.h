...
class PLUGINMAPVIEWRUNTIME_API UPluginMapViewComponent : public UMeshComponent, public IInterface_CollisionDataProvider
{
	GENERATED_BODY()
...
public:
	...
	UPluginMapView* GetPluginMapView()
	{
		return PluginMapView;
	}

	/** Returns PluginMapView asset object name  */
	FString GetPluginMapViewAssetName() const;
	...
	TArray< struct FPluginMapViewVertex > GetRawMeshVertices() const
	{
		return Vertices;
	}
	...
	/**
	* Returns PluginMapView Default Material if a valid one is found in plugin's content folder.
	* Otherwise , it returns the default surface 3d material.
	*/
	UMaterialInterface* GetDefaultMaterial() const
	{
		return PluginMapViewDefaultMaterial != nullptr ? PluginMapViewDefaultMaterial : UMaterial::GetDefaultMaterial(MD_Surface);
	}
	...
	*/
	...
	UFUNCTION(BlueprintCallable, Category = "PluginMapView")
		void SetPluginMapView(UPluginMapView* NewPluginMapView, bool bClearPreviousMeshIfAny = false, bool bRebuildMesh = false);
...
protected:
	...
	UPROPERTY(EditAnywhere, Category = "PluginMapView")
		UPluginMapView* PluginMapView;

	UPROPERTY(EditAnywhere, Category = "PluginMapView")
		FPluginMapViewMeshBuildSettings MeshBuildSettings;

	UPROPERTY(EditAnywhere, Category = "PluginMapView")
		FPluginMapViewCollisionSettings CollisionSettings;

	//** Physics data for mesh collision. */
	UPROPERTY(Transient)
		UBodySetup* PluginMapViewBodySetup;
...
protected:
	...
	/** Cached PluginMapView DefaultMaterial */
	UPROPERTY()
		UMaterialInterface* PluginMapViewDefaultMaterial;
