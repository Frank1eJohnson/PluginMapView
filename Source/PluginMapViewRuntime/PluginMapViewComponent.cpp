...
FPrimitiveSceneProxy* UPluginMapViewComponent::CreateSceneProxy()
{
	...
	if( HasValidMesh() )
	{
		PluginMapViewSceneProxy = new FPluginMapViewSceneProxy( this );
		PluginMapViewSceneProxy->Init( this, Vertices, Indices );
	}
	
	return PluginMapViewSceneProxy;
}


int32 UPluginMapViewComponent::GetNumMaterials() const
{
...
void UPluginMapViewComponent::GenerateMesh()
{
	...
	if( PluginMapView != nullptr )
	{
		...
		const auto& Roads = PluginMapView->GetRoads();
		const auto& Nodes = PluginMapView->GetNodes();
		const auto& Buildings = PluginMapView->GetBuildings();
				...
				if( bWant3DBuildings && Building.Height > KINDA_SMALL_NUMBER )
				{
					...
					else
					{
						...
						for( int32 PointIndex = 0; PointIndex < Building.BuildingPoints.Num(); ++PointIndex )
						{
							...
							FPluginMapViewVertex& NewVertex = *new( this->Vertices )FPluginMapViewVertex();
...
void UPluginMapViewComponent::AddThick2DLine( const FVector2D Start, const FVector2D End, const float Z, const float Thickness, const FColor& StartColor, const FColor& EndColor, FBox& MeshBoundingBox )
{
	...
	FPluginMapViewVertex& BottomLeftVertex = *new( Vertices )FPluginMapViewVertex();
	...
	FPluginMapViewVertex& BottomRightVertex = *new( Vertices )FPluginMapViewVertex();
	...
	FPluginMapViewVertex& TopRightVertex = *new( Vertices )FPluginMapViewVertex();
	...
	FPluginMapViewVertex& TopLeftVertex = *new( Vertices )FPluginMapViewVertex();
...
void UPluginMapViewComponent::AddTriangles( const TArray<FVector>& Points, const TArray<int32>& PointIndices, const FVector& ForwardVector, const FVector& UpVector, const FColor& Color, FBox& MeshBoundingBox )
{
	const int32 FirstVertexIndex = Vertices.Num();

	for( FVector Point : Points )
	{
		FPluginMapViewVertex& NewVertex = *new( Vertices )FPluginMapViewVertex();
